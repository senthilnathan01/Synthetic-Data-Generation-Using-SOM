...
# Modified Version: Palaniappan Ramu (palramu@iitm.ac.in)
...
def som_hitmap(vis, hits, comp, clr):  # Latest updated on 13 Sept
    ...
    nnodes = vis.nnodes
    colr = clr
    hit_size = hits/max(hits)         # hit_size for plotting red hex on each figure
    ...
    for i in range(ndim):
        ...
        if i==ndim-1:
            hover = HoverTool(tooltips=[("Hits","@Hits")], toggleable=False)
            for j in range(ndim):
                ...
                fig_obj[j].add_tools(hover)
                fig_obj[j].hex_tile(q='q', r='r', scale='Hit_scale', source=src_des_var, fill_color=colr, line_color='black') # black
        p.toolbar.logo = None
